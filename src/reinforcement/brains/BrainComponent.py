...
def conv_layer(input_block,filter_num,kernel_size,batch_norm = True,add_skip_con = False):
    ...
    x = Conv2D(filter_num,kernel_size,activation = 'linear',padding = 'same',data_format="channels_first", use_bias = False, kernel_regularizer=regularizers.l2(0.0001))(input_block)
    if batch_norm:
...
def input_conv_layer():
    ...
    x = conv_layer(input_block = main_input,filter_num = 75 , kernel_size= (4,4))
    return main_input , x

def residual_tower (input_block,res_number = 19): 
    x = residual_layer(input_block=input_block,filter_num=75,kernel_size=(4,4))
    for num in range(res_number-1):
        x = residual_layer(input_block=x,filter_num=75,kernel_size=(4,4))
    return x
...
def value_head (input_block):
    ...
    x = Flatten()(x)
    x = Dense(20,activation='linear',use_bias = False, kernel_regularizer=regularizers.l2(0.0001))(x)
    x = ReLU()(x)
